...
import type { EventWithRole } from "@/db/events";
...
type EventSettingsFormValues = z.input<typeof updateEventSchema>;
...
export function EventSettingsForm({ event }: EventSettingsFormProps) {
  ...
  const onSubmit = (values: EventSettingsFormValues) => {
    startTransition(async () => {
      const result = await updateEventAction({ eventId: event.id }, values);

      if (result.error) {
        if (result.fieldErrors) {
          Object.entries(result.fieldErrors).forEach(([field, message]) => {
            form.setError(field as keyof EventSettingsFormValues, { message });
          });
        } else {
