...
import { MAX_EVENT_TEAMS } from "@/services/constants";
...
import { CreateEventTeamForm } from "./create-event-team-form";
...
export async function generateMetadata({
  ...
  return {
    title: `New Team - ${result.data.name}`,
    description: `Create a team for ${result.data.name}`,
  };
}
...
async function CreateTeamContent({
  eventId,
  userId,
  eventName,
}: {
  ...
  const result = await getEventTeams(userId, eventId);
  const teams = result.data ?? [];

  if (teams.length >= MAX_EVENT_TEAMS) {
    return (
      <div className="space-y-4 md:space-y-6">
        <LeagueBreadcrumb
          items={[
        ...
        <AtLimitMessage
          ...
          description={`This event has reached the maximum of ${MAX_EVENT_TEAMS} teams.`}
        />
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-muted-foreground text-sm md:text-base">
          Set up a new team for your event
        </p>
      </div>
      <CreateEventTeamForm eventId={eventId} />
    </div>
  );
}

export default async function NewEventTeamPage({ params }: PageProps) {
  const session = await auth.api.getSession({
    headers: await headers(),
  });
  if (!session) {
