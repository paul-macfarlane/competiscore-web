...
import { EventInvitationWithDetails } from "@/db/event-invitations";
import { EVENT_ROLE_LABELS } from "@/lib/shared/roles";
import { Check, Copy, Link, Trash2, User } from "lucide-react";
import { useState, useTransition } from "react";

import { cancelEventInvitationAction } from "../../../actions";

interface EventPendingInvitationsListProps {
  invitations: EventInvitationWithDetails[];
  eventId: string;
}

export function EventPendingInvitationsList({
  invitations,
  eventId,
}: EventPendingInvitationsListProps) {
  ...
  const handleCancel = (invitationId: string) => {
    ...
    startTransition(async () => {
      const result = await cancelEventInvitationAction({
        invitationId,
        eventId,
      });
      if (result.error) {
  ...
  const handleCopyLink = async (token: string, invitationId: string) => {
    const inviteUrl = `${window.location.origin}/events/invite?token=${token}`;
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary">
                  {EVENT_ROLE_LABELS[
                    invitation.role as keyof typeof EVENT_ROLE_LABELS
                  ] ?? invitation.role}
                </Badge>
              </div>
