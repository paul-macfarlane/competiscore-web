...
- **Framework**: Next.js 16 with Turbopack, React 19 with React Compiler
- **Database**: PostgreSQL via Neon (serverless in production, node-postgres in dev), Drizzle ORM
- **Auth**: Better Auth (Discord & Google OAuth)
- **UI**: ShadCN (Radix UI), Tailwind CSS 4
- **Validation**: Zod 4, React Hook Form with `@hookform/resolvers/zod`
- **Testing**: Vitest
...
- Components that are re-used across multiple pages should be in the `src/components` directory.
...
- Service-layer constants (field length limits, max counts) should be in `src/services/constants.ts` and reused by both validators and services.
...
- Database functions should support being run on their own AND within a transaction via the `DBOrTx` pattern:
  ```typescript
  export async function getLeagueById(
    leagueId: string,
    dbOrTx: DBOrTx = db, // defaults to standalone, accepts transaction
  ): Promise<League | undefined>;
  ```
  The `DBOrTx` type, `db` instance, and `withTransaction()` helper are exported from `src/db/index.ts`.
...
- Database Types should use DrizzleORM's `InferSelectModel` and `InferInsertModel` functions and be exported from `src/db/schema.ts`:
  ```typescript
...
- Server components that fetch data should be async functions and utilize [Streaming](https://nextjs.org/docs/app/getting-started/fetching-data#streaming) to fetch data from the server. A `loading.tsx` file can be used for route segments (layouts and pages), but if the loading is more granular, Suspense can be used to handle the loading state.
...
### Client-Side Action Result Handling

Forms should handle `ServiceResult` responses using `useTransition` and `react-hook-form`:
...
- Field-level errors go to `form.setError()` for inline display
- Root-level errors go to `toast.error()` via sonner
...
All service functions return `Promise<ServiceResult<T>>` defined in `src/services/shared.ts`:
...
- On success: `{ data: T }`
- On validation failure: `{ error: "Validation failed", fieldErrors: { fieldName: "message" } }`
- On business logic error: `{ error: "Error message" }`

Use `formatZodErrors(zodError)` from `src/services/shared.ts` to convert Zod errors to `fieldErrors`.
...
After input validation, services check permissions in this order:

1. **League membership**: `getLeagueMember(userId, leagueId)` - verify user is a member
2. **Role-based permissions**: `canPerformAction(role, LeagueAction.X)` from `src/lib/shared/permissions.ts`
3. **Suspension check**: `isSuspended(member)` from `src/services/shared.ts` - verify user is not suspended
4. **Business rules**: domain-specific checks (duplicate detection, limit enforcement, etc.)

For team operations, use `canPerformTeamAction(role, TeamAction.X)`.
...
When a service function needs atomic multi-step operations, use `withTransaction()`:

```typescript
const result = await withTransaction(async (tx) => {
  const newLeague = await dbCreateLeague(parsed.data, tx);
  await createLeagueMember({ userId, leagueId: newLeague.id, role }, tx);
  return newLeague;
});
return { data: result };
```

For service functions that need to work both standalone and within a caller's transaction, accept an optional `dbOrTx` parameter:
...
We use Vitest for unit testing. Run tests with `pnpm test`.

### Test Organization

- Test files are colocated with source: `src/services/leagues.test.ts` tests `src/services/leagues.ts`
- Service tests mock all database and external dependencies at the module level
- Use `TEST_IDS` from `src/services/test-helpers.ts` for consistent valid UUIDs across tests
...
describe("createLeague", () => {
  ...
  it("returns validation error for invalid input", async () => {
    const result = await createLeague(TEST_IDS.USER_ID, { name: "" });
...
- **Only verify outputs** - never use `toHaveBeenCalled` or `toHaveBeenCalledWith`
- **Mock at module boundaries** - use `vi.mock()` for DB functions, `vi.mocked()` for type-safe return values
- **Clear mocks between tests** - always use `beforeEach(() => vi.clearAllMocks())`
- **Mock transactions** - use `vi.mock("@/db/index", () => ({ withTransaction: vi.fn((cb) => cb({})) }))`
- **Test all paths** - validation errors, permission failures, business rule violations, and success cases
